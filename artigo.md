...
Para a caracterização da regulação transcricional dos 49 candidatos LRR-RLP identificados em *S. lycopersicum*, foram extraídas sequências genômicas de 2.000 pares de bases (pb) a montante do sítio de início da transcrição anotado de cada gene. As coordenadas genômicas foram obtidas do arquivo de anotação GFF3 do genoma de referência de *Solanum lycopersicum* ITAG4.0 (SGN; Fernandez-Pozo et al., 2015), e as sequências foram recuperadas via API NCBI e-Utilities (`efetch`, banco de dados `nuccore`) com mapeamento dos cromossomos SL4.0 para os respectivos accessions GenBank (CM001064.3–CM001075.3). As sequências resultantes (49 × 2.000 pb) foram exportadas em formato FASTA multi-sequência para submissão à análise de elementos cis. Para os sete genes LRR-RLP focais (*SlRLP1* a *SlRLP7*), uma análise preliminar independente foi realizada com extração via `samtools faidx` e `bedtools flank` (opções `-l 2000 -s`; Quinlan e Hall, 2010) diretamente do genoma ITAG4.0 indexado no servidor.

### 2.2 Predição de Elementos Cis (PlantCARE)

As 49 sequências promotoras (2 kb upstream) foram submetidas ao servidor PlantCARE (*Plant Cis-Acting Regulatory Elements*; Lescot et al., 2002) para identificação e classificação de motivos cis-regulatórios em análise batch (job PlantCARE_28900; 98.000 nt totais). O PlantCARE emprega matrizes de peso de posição (PWM) e busca por homologia de sequência contra um banco de dados curado de elementos regulatórios vegetais, sendo amplamente utilizado em estudos genome-wide de famílias gênicas em Solanaceae (Noman et al., 2022; Yang et al., 2023). Para o mapeamento de hits às sequências individuais a partir do resultado concatenado, foi desenvolvido script Python de pós-processamento que remapeia a posição absoluta no bloco de 98 kb ao gene de origem pelo cálculo `gene_index = (posição − 1) ÷ 2000`. Os elementos identificados foram filtrados pelo nível de confiança (*Core* e *Extended*), excluindo-se elementos de função desconhecida ou sem embasamento experimental. A contagem de ocorrências por gene e por categoria funcional foi tabulada em planilha estruturada (`plantcare_counts_49genes.csv`) e visualizada em mapa de calor gerado com o pacote `pheatmap` (v1.0.12; Kolde, 2019) no ambiente R (versão 4.3.2). Os scripts encontram-se em `analyses/02_promoter_cis/`.
...
Para avaliar o modo de evolução dos genes LRR-RLP duplicados em *S. lycopersicum*, foram identificados 13 pares de genes parálogos a partir da análise da organização genômica dos 49 candidatos (clustering em tandem e blocos segmentais identificados na análise de sintenia, Seção 7). As sequências codificantes (CDS) de cada par foram obtidas do proteoma ITAG4.0 e alinhadas com o programa MAFFT (v7.526, parâmetros `--auto`; Katoh e Standley, 2013). As razões de substituição não-sinônima por sinônima (Ka/Ks, também referido como dN/dS) foram calculadas pelo módulo `Bio.codonalign.codonseq.cal_dn_ds` da biblioteca BioPython (Cock et al., 2009), implementando o método de Nei-Gojobori (NG86; Nei e Gojobori, 1986). Codons de parada (TAA, TAG, TGA) foram removidos dos alinhamentos antes do cálculo, e colunas com gaps ou bases ambíguas (N) foram excluídas. Pares com Ka/Ks < 1 foram interpretados como sob seleção purificadora (conservação funcional); Ka/Ks > 1 indica seleção positiva (neofuncionalização); Ka/Ks ≈ 1 sugere evolução neutra. Os 13 pares identificados distribuem-se nos seguintes cromossomos: Chr01 (5 pares em tandem: Solyc01g005730 a Solyc01g106500), Chr06 (2 pares em tandem), Chr07 (4 pares em tandem, cluster compacto), Chr12 (2 pares em tandem) e Chr02 (1 par segmental: Solyc02g072250 × Solyc02g092040, correspondente a SlRLP5 × SlRLP6). Os scripts de execução e o arquivo `gene_pairs.tsv` encontram-se em `analyses/04_kaks/`.
...
A co-ocorrência de elementos de resposta a ABA (ABRE, MBS, G-box) e de defesa (ERE, TC-rich repeats, TCA-element) nos promotores de múltiplos genes LRR-RLP sugere sobreposição regulatória entre as vias de sinalização de estresse abiótico (seca) e biótico (defesa antiviral), padrão consistente com o fenômeno descrito como crosstalk SA-ABA-ET em plantas sob estresses combinados (Verma et al., 2016; Cai et al., 2024).

A análise dos **49 candidatos LRR-RLP** (job PlantCARE_19728; 7.762 hits totais; 101 categorias de elementos) confirmou e expandiu os resultados preliminares dos sete genes focais (Tabela 1 expandida em `plantcare_counts_49genes.csv`). Os elementos mais prevalentes nos 49 promotores foram: **CGTCA-motif/TGACG-motif** (JA, 36/49 genes), **ABRE** (ABA, 33/49), **ERE** (etileno/defesa, 32/49), **W box** (SA, 29/49), **GT1-motif** (luz, 29/49) e **TCA-element** (SA, 28/49). O elemento **as-1** (TGACG-related, ativador por SA/JA; 36/49 genes) é particularmente relevante: sua alta prevalência em genes LRR-RLP corrobora a hipótese de que a família como um todo está sob regulação por vias hormonais de defesa, não apenas os sete genes focais. A análise completa está disponível em `analyses/02_promoter_cis/plantcare_results_49genes.tab`.
...
A ausência completa do domínio *Pkinase* (PF00069) em todos os 48 genes analisados confirma, de forma inequívoca, a identidade desses genes como RLPs e não como RLKs (*Receptor-Like Kinases*), cumprindo o critério estrutural definitório da classe (Shiu e Bleecker, 2001). A figura de arquitetura de domínios dos 49 LRR-RLPs, com destaque para os sete genes focais (*SlRLP1* a *SlRLP7*, marcados com asterisco e backbone em cinza escuro), foi gerada com os pacotes `ggplot2` e `ggnewscale` em R, em ordem cromossômica. A figura está disponível como `domain_architecture.pdf` no diretório `analyses/06_domain_architecture/`.
...
A análise dos blocos collineares revelou que **todos os sete genes LRR-RLP focais estão inseridos em blocos sintênicos conservados com pelo menos uma das espécies comparadas** (Tabela 3). Cinco genes (*SlRLP1*, *SlRLP2*, *SlRLP3*, *SlRLP5*, *SlRLP6*) possuem ortólogos sintênicos confirmados tanto em *C. annuum* quanto em *S. tuberosum*; *SlRLP4* (Solyc12g042760) apresenta bloco conservado com batata (bloco 1417; PGSC0003DMG400028627); e *SlRLP7* (Solyc10g007830) possui bloco com pimenta (bloco 209; T459_24802). A conservação sintênica dos genes CLV2-like (*SlRLP1*/Solyc05g055190 e *SlRLP2*/Solyc03g112680) em blocos independentes com ambas as espécies é consistente com a duplicação segmental inferida pela análise Ka/Ks (Seção 4).

**Tabela 3. Blocos sintênicos MCScanX contendo os sete genes LRR-RLP focais de *S. lycopersicum*, com ortólogos identificados em *C. annuum* (pimenta) e *S. tuberosum* (batata).**

| Gene focal | Solyc ID | Bloco (pimenta) | Ortólogo pimenta | Bloco (batata) | Ortólogo batata |
|---|---|---|---|---|---|
| *SlRLP1* (CLV2) | Solyc05g055190 | 920 | T459_15510 (e=2×10⁻¹⁴²) | 1596 | PGSC0003DMG400023498 (e=0) |
| *SlRLP2* | Solyc03g112680 | 675 | T459_10148 (e=1×10⁻¹⁰⁶) | 1495 | PGSC0003DMG400018163 (e=0) |
| *SlRLP3* (RIC7) | Solyc05g009990 | 297 | T459_26975 (e=1×10⁻²²¹) | 1599 | PGSC0003DMG400000819 (e=2×10⁻²⁴³) |
| *SlRLP4* (TMM) | Solyc12g042760 | — | — | 1417 | PGSC0003DMG400028627 (e=2×10⁻³¹²) |
| *SlRLP5* (SNC2/3) | Solyc02g072250 | 537 | T459_05381 (e=1×10⁻²⁶⁵) | 1427/1483 | PGSC0003DMG401028427/DMG400026750 |
| *SlRLP6* | Solyc02g092040 | 513 | T459_06566 (e=3×10⁻¹⁸⁶) | 1428 | PGSC0003DMG400043061 (e=8×10⁻¹⁹⁷) |
| *SlRLP7* | Solyc10g007830 | 209 | T459_24802 (e=2×10⁻²²⁷) | — | — |

Os blocos collineares foram depositados em `analyses/07_synteny_solanaceae/mcscan_results/rlp_synteny_blocks.txt`. A identificação de blocos sintênicos conservados entre tomate e batata tem implicações diretas para o mapeamento dos loci *Ty* de resistência a begomovírus presentes em *S. peruvianum* e introgressados em variedades comerciais de tomate, potencialmente co-localizando com genes LRR-RLP identificados neste estudo.
...
SAYERS EW et al. (2022) Database resources of the National Center for Biotechnology Information. *Nucleic Acids Research* 50:D20–D26.
...
*Documento atualizado em 2026-06-30. Status das seções:*
- *§2 PlantCARE 7 focais* ✅ *completo; §2 PlantCARE 49 genes* ⏳ *em processamento (job PlantCARE_28900, e-mail eulalio.santos@ufv.br)*
- *§3 Expressão* ✅ *completo (dados compilados GEO/SRA; validação TFGD pendente)*
- *§4 Ka/Ks* ✅ *completo — 13 pares, todos Ka/Ks < 1 (NG86; biopython), Tabela 2 inserida*
- *§5 RMSD/MolProbity* ⏳ *pendente — aguardando modelos PDB (AlphaFold DB ou Swiss-Model)*
- *§6 HMMER/Pfam* ✅ *completo — 2.094 hits, 48/49 genes anotados; figura 49 genes pronta para gerar no servidor*
- *§7 MCScanX sintenia* ✅ *completo — 1.766 blocos, Tabela 3 inserida com ortólogos dos 7 focais*
